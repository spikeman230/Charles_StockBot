...
if __name__ == "__main__":
    ...
    if MY_PORTFOLIO:
        ...
        for sym, data in MY_PORTFOLIO.items():
            ...
            curr_price = td['Close']; atr = td['ATR']; buy_price = data['buy_price']; roi_pct = ((curr_price - buy_price) / buy_price) * 100
    ...
    for cat, stocks in STOCK_DICT.items():
        ...
        for sym, name in stocks.items():
            ...
            if not cat_printed: msg_list.append(f"━━━━━━━━━━━━━━\n📂 【{cat}】\n━━━━━━━━━━━━━━\n"); cat_printed = True
            close = td['Close']; atr = td['ATR']; rsi = td['RSI']; vma5 = td['5VMA']
            ...
            predict_msg = "無特殊徵兆"
            if td['Volume'] > vma5 * 3 and rsi > 75: predict_msg = "💀【動能竭盡】異常爆量！"
            ...
            if sym_state["status"] == "REAL_HOLD": alert = f"💼 已列入持股防禦區 | 📍 防線: {sym_state['trailing_stop']:.1f}"
                ...
                if close < new_stop: alert = f"🩸【拔線離場】跌破防守線 {new_stop:.1f}！"; noc_state[sym] = {"status": "NONE"}
                else: noc_state[sym]["trailing_stop"] = new_stop; alert = f"🔥【波段抱牢】損益: {((close - sym_state['entry']) / sym_state['entry']) * 100:+.2f}% | 防守線: {new_stop:.1f}"
