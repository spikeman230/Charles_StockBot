...
if __name__ == "__main__":
    ...
    try:
        print("1?? 正在下載大盤與防空歷史數據 (近400天)...")
        fetcher.fetch_market_health_data(start_date, db)

        target_stocks = list(set([s for s in SCAN_LIST]))  # 保留完整股票代號
        print(f"?? 鎖定 {len(target_stocks)} 檔目標，準備進行歷史大補給！")
        ...
        for i, sym in enumerate(target_stocks, 1):
            ...
            time.sleep(0.8)  # 避免 API 限制

        print("\\\\n? 歷史戰情資料庫 (SQLite) 初始建置完成！")
